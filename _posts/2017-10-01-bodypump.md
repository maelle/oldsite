...
```
## Error in artists_info %>% split(artists_info$cluster) %>% purrr::map_chr(get_most_important_genre): could not find function "%>%"
```
...
```
## Error in overscope_eval_next(overscope, expr): object 'styles' not found
```
...
```
## Error in inherits(x, "list"): object 'cluster_music' not found
```
...
```
## Error in tbl_vars(y): object 'cluster_music' not found
```
...
```
## Error in FUN(X[[i]], ...): object 'style' not found
```
