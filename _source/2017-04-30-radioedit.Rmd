...
This time I got `r length(search_dates)/length(unique(programs_df$date))*100`% of programs!
...
This seems to support the idea that upper quantiles of duration are bigger at night. This shift of the distribution of duration at night compared to its distribution during daytime means that there are more long pieces at night... which is what the website claims, so we're good. Just for info, we are looking at `r nrow(dplyr::filter(programs_df, !is.na(duration), night))` pieces played at night and `r nrow(dplyr::filter(programs_df, !is.na(duration), !night))` pieces played during the day, that is to say a lot.
...
Now that we have `r nrow(programs)` pieces with a known speed out of `r nrow(programs_df)`, let's look at speed distribution depending on the hour of the day.
