---
layout: post
title: "Are #python users more likely to get into Slytherine?"
comments: true
---
...
Have you seen Mark Sellors' blog post series about writing command line utilities in R? It's a great one but I was a bit puzzled by his using randomness to assign houses in his sorting hat example.
...
In his repo, David Hood used accounts sampled from their posting on the #rstats hashtag, and uses their scores as a scale against which to rate any user. I had to take a different approach because I wanted to compare R against Python posting on a basis that wouldn't be R. I chose to use JavaScript posters as a reference set, because you can't randomly search Twitter users and even that wouldn't necessarily give me a "representative" set so I had to assume something, and I chose to assume the Sorting hat would have chosen JavaScript tweeps as a training set. I wanted more than 200 JavaScript tweeps and more than 100 R and Python tweeps, respectively, and asked for a bit more from Twitter API because I knew there'd be duplicates from people tweeting about more than one of these languages (like [Lynn Cherny](https://twitter.com/arnicas) for instance). I didn't do any sample size calculation. Here is how I got users:
...
get_score <- function(candidate, house , compare_with)
...
sort_user <- function(candidate, compare_with){
  ...
  slyth <- dplyr::bind_rows(compare_with, candidate) %>% 
    dplyr::arrange(slyth) %>% 
    dplyr::select(slyth,isSortee)
Sly <- round(which(slyth$isSortee)/nrow(slyth),3)

griff <- dplyr::bind_rows(compare_with, candidate) %>% 
  dplyr::arrange(griff) %>% 
  dplyr::select(griff,isSortee)
Gry <-  round(which(griff$isSortee)/nrow(griff),3)

huffl <- dplyr::bind_rows(compare_with, candidate) %>% 
  dplyr::arrange(huffl) %>% 
  dplyr::select(huffl,isSortee)
Huf <- round(which(huffl$isSortee)/nrow(huffl),3)

raven <- dplyr::bind_rows(compare_with, candidate) %>% 
  dplyr::arrange(raven) %>% 
  dplyr::select(raven,isSortee)
Rav <- round(which(raven$isSortee)/nrow(raven),3)

result <- data.frame(House = c("Slytherin", "Gryffindor", 
                               "Hufflepuff", "Ravenclaw"),
                     Proportion = c(Sly, Gry, Huf, Rav),
                     language = candidate$language,
         screen_name = candidate$screen_name) %>% 
  dplyr::arrange(-Proportion) %>%
  dplyr::mutate(Score = round(100 * Proportion/ sum(Proportion),1)
         )
result %>%
  head(n = 1) %>%
...
So now has come the crucial moment of truth... Are Python users more likely to get sorted into Slytherin? I'll leave statistical tests as an exercise to the reader and just do a visualizatio.
...
Oh nooo, what a non-intuitive result!  But maybe this means the R community is very social on Twitter? 