...
I first opened my table of `r nrow(us_airports)` airports. Then, I made the call to the Iowa Environment Mesonet.
...
```{r, echo = FALSE}
load("data/2017-11-16_wheretoliveus_winter_weather.RData")
load("data/2017-11-16_wheretoliveus_summer_weather.RData")

```

I quickly checked that there was "nearly no missing data" so I didn't remove any station nor day but if I were doing this analysis for something serious I'd do more checks including the time difference between measures for instance. Note that I end up with `r length(unique(summer_weather$station))` airports only, there was no measure for `r unique(us_airports$Name)[!unique(us_airports$Code) %in% unique(summer_weather$station)]`. Too bad!
