...
Now let's get to work on the sweetening of the texts at last!
...
modifiable_tweets <- dplyr::group_by(modifiable_tweets, saved_text) %>%
  dplyr::summarize(praise_template = unlist(purrr::map2(word, modifier, replace_all,
                                                  x = saved_text))[1])
```
